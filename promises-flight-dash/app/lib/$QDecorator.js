...
(function ( window ){
  ...
  // Decorate the $q service instance to add extra `spread()` and `resolve()` features
  ...
  // Partial application to build a resolve() function
    ...
    $provide.decorator('$q', ["$delegate", function ($delegate) {
      if ( angular.isUndefined( $delegate.spread )) {
        // Let's add a `spread()` that is very useful when using $q.all()
      ...
      if ( angular.isUndefined( $delegate.resolve )) {
        // Similar to $q.reject(), let's add $q.resolve() to easily make an
        // immediately-resolved promise ... this is useful for mock promise-
        // returning APIs.
